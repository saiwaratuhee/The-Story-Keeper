...
echo '<blockquote>';
    ...
    while($row = $result->fetch_assoc()) {
	    echo "<td>";
	    echo "<table>";
		echo '<tr><td>'.'<img src="'.$row["Image"].'"width="80%">'.'</td></tr><tr><td style="padding: 5px;">Title: '.$row["BookTitle"].'</td></tr><tr><td style="padding: 5px;">Author: '.$row["Author"].'</td></tr><tr><td style="padding: 5px;">Genre: '.$row["Genre"].'</td></tr><tr><td style="padding: 5px;">BDT'.$row["Price"].'</td></tr><tr><td style="padding: 5px;">
    ...
    while($row = $result->fetch_assoc()){
    	...
    	echo $row['BookTitle']."<br>BDT".$row['Price']."<br>";
    	echo "Quantity: ".$row['Quantity']."<br>";
    	echo "Total Price: BDT".$row['TotalPrice']."</td></tr>";
    	$total += $row['TotalPrice'];
    }
    echo "<tr><td style='text-align: right;background-color: #f2f2f2;''>";
    echo "Total: <b>BDT".$total."</b><center><form action='checkout.php' method='post'><input class='button' type='submit' name='checkout' value='CHECKOUT'></form></center>";
    echo "</td></tr>";
	echo "</table>";
	echo '</blockquote>';
...
if (isset($_POST['search'])) {
    ...
    $sql = "SELECT * FROM book WHERE BookTitle LIKE '%$searchQuery%' OR Author LIKE '%$searchQuery%' OR Genre LIKE '%$searchQuery%' OR Bookstore LIKE '%$searchQuery%'";
    ...
    if ($result) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            echo "<p>Title: " . $row['BookTitle'] . "<br>";
            echo "Author: " . $row['Author'] . "<br>";
			echo "Genre: " . $row['Genre']."<br>";
			echo "Location: " . $row['Bookstore']."<br>";

            $found = true; // Book found
        }
        if (!$found) {
            echo "Book Not Found"; // Book not found
        }
    } else {
...
<head>
    ...
    <style>
        .search-bar {
            width: 200px; /* Set desired width for the search bar */
			margin: auto;
        }
    </style>
</head>
    <?php
        echo '<form method="post" action="">
            <input type="text" name="search" style="width: 200px;" placeholder="Search for books...">
